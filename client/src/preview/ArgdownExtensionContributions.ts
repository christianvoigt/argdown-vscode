...
export class ArgdownExtensionContributions {
	...
	private readonly _plugins: Thenable<(argdownEngine: any) => any>[] = [];
	...
	public get argdownPlugins(): Thenable<(argdownEngine: any) => any>[] {
		this.ensureLoaded();
	...
	private ensureLoaded() {
		...
		for (const extension of vscode.extensions.all) {
			...
			if (contributes['argdown.previewScripts'] || contributes['argdown.previewStyles']) {
		...
		if (contributes['argdown.argdownPlugins']) {
			this._plugins.push(extension.activate().then(() => {
				if (extension.exports && extension.exports.extendArgdown) {
					return (argdownEngine: any) => extension.exports.extendArgdown(argdownEngine);
				}
				return (argdownEngine: any) => argdownEngine;
			}));
		}
	}
	...
	private tryLoadPreviewScripts(
		...
		this._scripts.push(...resolveExtensionResources(extension, contributes['argdown.previewScripts']));
	}
	...
	private tryLoadPreviewStyles(
		...
		this._styles.push(...resolveExtensionResources(extension, contributes['argdown.previewStyles']));
	}
}