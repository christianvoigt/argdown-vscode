...
import {
	...
	Diagnostic, DiagnosticSeverity, Range, InitializeParams, InitializeResult, ProposedFeatures, Proposed
	
} from 'vscode-languageserver';
import Uri from 'vscode-uri';
import {IArgdownSettings} from './IArgdownSettings';
import {exportHtml} from './commands/HtmlExport';
import { exportDot } from './commands/DotExport';
import { exportJSON } from './commands/JSONExport';
...
const EXPORT_HTML_COMMAND = "argdown.exportHtml";
const EXPORT_JSON_COMMAND = "argdown.exportJSON";
const EXPORT_DOT_COMMAND = "argdown.exportDot";
const RUN_COMMAND = "argdown.run";

const argdownCli = require('argdown-cli');
const argdownApp = argdownCli.app;

// Create a connection for the server. The connection uses Node's IPC as a transport
let connection = createConnection(ProposedFeatures.all);
argdownApp.logger.log = function (level: string, message: string) {
...
connection.onInitialize((params: InitializeParams & Proposed.WorkspaceFoldersInitializeParams): InitializeResult => {
...
const defaultSettings: IArgdownSettings = { 
	htmlDirectory: "html",
	htmlExportInput: "**/*{.argdown, .ad, .adn, .argdn}",
	dotDirectory: "dot",
	dotExportInput: "**/*{.argdown, .ad, .adn, .argdn}",
	jsonDirectory: "json",
	jsonExportInput: "**/*{.argdown, .ad, .adn, .argdn}",
...
async function validateTextDocument(textDocument: TextDocument): Promise <void> {
	...
	let result = argdownApp.run(['preprocessor', 'parse-input', 'build-model'], { input: text })
	let diagnostics: Diagnostic[] = [];
	if (result.parserErrors && result.parserErrors.length > 0) {
		for (var error of result.parserErrors) {
			var start = { line: error.token.startLine - 1, character: error.token.startColumn - 1 };
			var end = { line: error.token.endLine - 1, character: error.token.endColumn }; //end character is zero based, exclusive
...
connection.onExecuteCommand(async (params) => {
	if (params.command === EXPORT_HTML_COMMAND) {
		if(hasWorkspaceFolderCapability){
	...
	else if (params.command === EXPORT_JSON_COMMAND) {
		if (hasWorkspaceFolderCapability) {
			if (!workspaceFolders ||  workspaceFolders.length == 0) {
				connection.console.log("No workspace folder found.");
			}
	...
	else if (params.command === EXPORT_DOT_COMMAND) {
		if (hasWorkspaceFolderCapability) {
			if (!workspaceFolders ||  workspaceFolders.length == 0) {
				connection.console.log("No workspace folder found.");
			}
		...
		for (var workspaceFolder of workspaceFolders) {
			...
			let config = argdownApp.loadConfig(configPath);
			if(config !== null){
				if(!config.rootPath){
